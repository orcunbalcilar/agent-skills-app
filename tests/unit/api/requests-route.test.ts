...
import { GET as getRequest, DELETE as withdrawRequest } from "@/app/api/v1/requests/[requestId]/route";
...
describe("POST /api/v1/skills/[id]/requests", () => {
  ...
  it("should create request and notify owners", async () => {
    ...
    expect(dispatchNotification).toHaveBeenCalledWith(
      "CHANGE_REQUEST_SUBMITTED", ["o1"], expect.objectContaining({ requestId: "r1" })
    );
  });
...
describe("GET /api/v1/requests/[requestId]", () => {
  ...
  it("should return request details", async () => {
    vi.mocked(prisma.changeRequest.findUnique).mockResolvedValue({
      id: "r1", title: "Fix", skill: { id: "s1", name: "skill" },
    } as never);
...
describe("POST /api/v1/requests/[requestId]/approve", () => {
  ...
  it("should approve request via transaction", async () => {
      ...
      const tx = {
        changeRequest: {
          findUnique: vi.fn().mockResolvedValue({
            id: "r1", status: "OPEN", skillId: "s1", requesterId: "requester1",
    ...
    vi.mocked(prisma.changeRequest.findUnique).mockResolvedValue({
      id: "r1", requesterId: "requester1", skillId: "s1",
    } as never);
    ...
    expect(res.status).toBe(200);
    expect(dispatchNotification).toHaveBeenCalledWith(
      "CHANGE_REQUEST_APPROVED", ["requester1"], expect.objectContaining({ requestId: "r1" })
    );
  });
  ...
  it("should return 400 when request is not OPEN", async () => {
    vi.mocked(prisma.$transaction).mockImplementation(async (cb: unknown) => {
      const tx = {
        changeRequest: {
          findUnique: vi.fn().mockResolvedValue({
            id: "r1", status: "APPROVED", skill: { owners: [] },
          }),
        },
      };
      return (cb as (t: typeof tx) => Promise<unknown>)(tx);
  ...
  it("should return 403 when not owner and not admin", async () => {
    vi.mocked(prisma.$transaction).mockImplementation(async (cb: unknown) => {
      const tx = {
        changeRequest: {
          findUnique: vi.fn().mockResolvedValue({
            id: "r1", status: "OPEN", skill: { owners: [{ userId: "other" }] },
          }),
        },
      };
      return (cb as (t: typeof tx) => Promise<unknown>)(tx);
...
describe("POST /api/v1/requests/[requestId]/reject", () => {
  ...
  it("should reject request and notify requester", async () => {
    vi.mocked(prisma.changeRequest.findUnique).mockResolvedValue({
      id: "r1", status: "OPEN", requesterId: "req1", skillId: "s1",
      skill: { name: "skill", owners: [{ userId: "u1" }] },
    } as never);
    vi.mocked(prisma.changeRequest.update).mockResolvedValue({ id: "r1", status: "REJECTED" } as never);
    ...
    expect(res.status).toBe(200);
    expect(dispatchNotification).toHaveBeenCalledWith(
      "CHANGE_REQUEST_REJECTED", ["req1"], expect.objectContaining({ requestId: "r1" })
    );
  });
