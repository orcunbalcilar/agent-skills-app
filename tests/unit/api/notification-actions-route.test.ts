...
describe("Rate limit tests", () => {
  it("should return rate limit for read-all", async () => {
    ...
    const req = new NextRequest("http://localhost/api/v1/notifications/read-all", { method: "POST" });
  ...
  it("should return rate limit for preferences", async () => {
    const limitResponse = new Response(JSON.stringify({ error: "Too Many Requests" }), { status: 429 });
    vi.mocked(checkLimit).mockReturnValue(limitResponse as never);
    const req = new NextRequest("http://localhost/api/v1/notifications/preferences", { method: "PATCH", body: JSON.stringify({}) });
