...
describe("useCreateTag", () => {
  ...
  it("should create a tag", async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(mockFetch).toHaveBeenCalledWith("/api/v1/tags", expect.objectContaining({ method: "POST" }));
  });
...
describe("useDeleteTag", () => {
  ...
  it("should delete a tag", async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(mockFetch).toHaveBeenCalledWith("/api/v1/tags/t1", expect.objectContaining({ method: "DELETE" }));
  });
