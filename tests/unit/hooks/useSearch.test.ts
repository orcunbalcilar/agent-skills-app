...
describe('useSearch', () => {
  ...
  it('should include tags and sort in search params', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: () =>
        Promise.resolve({ data: [], meta: { page: 1, pageSize: 12, total: 0, totalPages: 0 } }),
    });

    renderHook(() => useSearch({ q: 'hello', tag: ['js'], sort: 'recent', page: 2 }), {
  ...
  it('should not search when query is empty', () => {
    const { result } = renderHook(() => useSearch({ q: '' }), { wrapper: createWrapper() });
