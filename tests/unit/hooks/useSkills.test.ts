...
describe("useSkills", () => {
  ...
  it("should fetch skills with search params", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ data: [], meta: { page: 2, pageSize: 5, total: 0, totalPages: 0 } }),
    });

    const { result } = renderHook(
      () => useSkills({ q: "test", sort: "popular", page: 2, pageSize: 5, tag: ["js", "python"], status: "RELEASED", ownerId: "u1" }),
