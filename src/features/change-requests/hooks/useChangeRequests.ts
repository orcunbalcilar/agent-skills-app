...
export function useSubmitChangeRequest(skillId: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["change-requests", skillId] }); },
  });
}
...
export function useApproveChangeRequest(skillId: string) {
  ...
  return useMutation({
    mutationFn: (requestId: string) =>
      postRequest(`/api/v1/requests/${requestId}/approve`, "POST"),
    onSuccess: () => {
...
export function useRejectChangeRequest(skillId: string) {
  ...
  return useMutation({
    mutationFn: (requestId: string) =>
      postRequest(`/api/v1/requests/${requestId}/reject`, "POST"),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["change-requests", skillId] }); },
  });
}
...
export function useWithdrawChangeRequest(skillId: string) {
  ...
  return useMutation({
    mutationFn: (requestId: string) =>
      postRequest(`/api/v1/requests/${requestId}`, "DELETE"),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["change-requests", skillId] }); },
  });
}
