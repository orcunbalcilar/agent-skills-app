...
function validateSkillEntries(entries: SkillFolderEntry[]): { error: string } | { data: Record<string, unknown>; files: SkillFolderEntry[] } {
  ...
  if (!skillMdEntry) return { error: "Zip must contain a SKILL.md file at the skill root" };
  ...
  try {
    ...
    return { error: err instanceof Error ? err.message : "Invalid SKILL.md frontmatter" };
  }
  ...
  if (rootPrefix) {
    ...
    if (dirName !== metadata.name) {
      return { error: `Directory name "${dirName}" must match skill name "${metadata.name}"` };
    }
  }
