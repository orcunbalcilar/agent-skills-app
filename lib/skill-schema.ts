...
export function parseSkillMd(content: string): {
  ...
  if (!match) {
    return {
      success: false,
      error: 'SKILL.md must contain YAML frontmatter between --- delimiters',
    };
  }
...
function checkDirectories(relativePaths: string[]): string[] {
  ...
  for (const rp of relativePaths) {
    ...
    if (parts.length > 1 && !ALLOWED_DIRS.has(parts[0]) && !warned.has(parts[0])) {
      warned.add(parts[0]);
      warnings.push(
        `Non-standard directory "${parts[0]}". Allowed by spec: scripts/, references/, assets/`,
      );
    }
  }
  return warnings;
...
export function validateSkillFolder(entries: SkillFolderEntry[]): {
  ...
  const relativePaths = entries.map((e) => (rootPrefix ? e.path.slice(rootPrefix.length) : e.path));
